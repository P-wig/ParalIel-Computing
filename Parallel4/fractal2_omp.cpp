/*
Compile: g++ -fopenmp -o fractal2_omp fractal2_omp.cpp

Run: ./fractal2_omp 1024 4
     ./fractal2_omp 1024 8

Parallelizing the inner loop is less efficient than parallelizing the outer loop.
*/
...
static void fractal( const int width, unsigned char* const pic, const int threads ){
  ...
  for (int row = 0; row < width; row++) {  // rows
    //double cx = xMin;
#   pragma omp parallel for default( none ) num_threads( threads ) shared( width, pic, dw, xMin, cy, row )
    for (int col = 0; col < width; col++) {  // columns
        double cx = xMin + ( dw * col );
        double x = cx;
        double y = cy;
        double x2, y2;
        int count = 256;
