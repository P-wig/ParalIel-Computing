...
Fifth. Compile!
   - Compile: nvcc -o fractal_float fractal_float.cu
   - Compile explicitly: nvcc -o fractal_float fractal_float.cu --compiler-bindir="C:/Program Files (x86)/Microsoft Visual Studio/2022/BuildTools/VC/Tools/MSVC/14.42.34433/bin/Hostx64/x64"

Sixth. Run!
   - If still using [Developer Command Prompt for VS 2022]
     - fractal_float.exe 4096
   - If using terminal
     - ./fractal_float 4096
*/
...
static __global__ void fractal( const int width, unsigned char* const pic ){
  ...
  if( i < (width * width) ) {
      ...
      const float xCenter = -0.663889302;
      const float yCenter =  0.353461972;

      // compute pixels of image
      const float xMin = xCenter - scale;
      const float yMin = yCenter - scale;
      const float dw = 2.0f * scale / width;

      float cy = yMin + (row * dw);
      float cx = xMin + (col * dw);
...
int main(int argc, char* argv []){
  std::cout << "Fractal with float precision";
